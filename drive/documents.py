...
# POST /drive/documents/{document_id}/tabs/{name}: Creates new empty tab in the document
@router.post("/drive/documents/{document_id}/tabs/{name}")
async def create_tab(document_id: str, name: str, docs_service=Depends(get_docs_service)):
    """
    Create a new empty tab within an existing document.
    
    Example input request:
        POST /drive/documents/1a-28yTY23NuCa7vmyMABGgRDCErW58Q99F_2o9ZePGo/tabs/NewTab
    
    Google API request sent:
        {
            "requests": [
                {"insertTab": {"tabProperties": {"title": "NewTab"}}}
            ]
        }
        (POST to docs_service.documents().batchUpdate)
    """
    try:
        body = {
            "requests": [
                {"insertTab": {"tabProperties": {"title": name}}}
            ]
        }
...
# GET /drive/documents/{document_id}/tabs/{name}: Returns existing tab content from the document
@router.get("/drive/documents/{document_id}/tabs/{name}")
async def get_tab(document_id: str, name: str, docs_service=Depends(get_docs_service)):
    """
    Get a specific tab from a document by its name.
    
    Example input request:
        GET /drive/documents/1a-28yTY23NuCa7vmyMABGgRDCErW58Q99F_2o9ZePGo/tabs/NewTab
    
    Google API request sent:
        docs_service.documents().get(documentId=document_id, includeTabsContent=true)
    """
    try:
        # Get document with all tabs content included
        ...
        for tab in tabs:
            if tab.get("tabProperties", {}).get("title") == name:
                return tab
        
        raise HTTPException(status_code=404, detail=f"Tab '{name}' not found.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

# DELETE /drive/documents/{document_id}/tabs/{name}: Delete existing tab and content from the document
@router.delete("/drive/documents/{document_id}/tabs/{name}")
async def delete_tab(document_id: str, name: str, docs_service=Depends(get_docs_service)):
    """
    Delete a specific tab from a document by its name.
    
    Example input request:
        DELETE /drive/documents/1a-28yTY23NuCa7vmyMABGgRDCErW58Q99F_2o9ZePGo/tabs/NewTab
    
    Google API request sent:
        {
            "requests": [
                {"deleteTab": {"tabId": "tab_id_found_by_name"}}
            ]
        }
        (POST to docs_service.documents().batchUpdate)
    """
    try:
        # First, get the document to find the tab ID by name
        document = docs_service.documents().get(
            documentId=document_id, 
            includeTabsContent=True
        ).execute()
        
        # Find the tab ID by name
        tabs = document.get("tabs", [])
        tab_id = None
        for tab in tabs:
            if tab.get("tabProperties", {}).get("title") == name:
